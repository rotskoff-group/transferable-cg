    ...
    if multiple_protein_names is not None:
        ...
        for protein_name in multiple_protein_names:
            ...
            if is_individual_dataset_made:
                all_sp_filenames.append(f"{root_data_folder_name}/{protein_name}/dataset.hdf5")
                continue

            os.makedirs(save_folder_name, exist_ok=True)

            cg_config_sp = copy.deepcopy(cg_config)
            OmegaConf.update(cg_config_sp,
                             "cg_model_args.protein_name",
                             str(protein_name))
            
            #Creating cg_model automatically creates dataset
            cg_model = create_cg_model(save_folder_name, root_data_folder_name, cg_config_sp)
        ...
        if concatenate_datasets:
            concatenate_cg_datasets(all_sp_filenames, root_save_folder_name, 
