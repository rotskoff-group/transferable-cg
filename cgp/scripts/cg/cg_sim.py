...
import hydra
import torch
from cgp.sim import OVRVO, generate_trajectory, CGSimModel
...
def main(cfg):
    ...
    u_model.eval()
    u_model.store_features(
        batch_size, global_args["pdb_file"], nn_config, prior_config, edge_args
    )
    cfg.nn = nn_config
    ...
    masses = get_cg_bead_masses(
        pdb.atom_slice(backbone).topology
    )  # units of amu aka Dalton
    ...
    if len(chk_files) > 0:
        ...
        print(
            f"Found {len(chk_files)} checkpoint files. Resuming from {chk_files[-1]}",
            flush=True,
        )
        chk = torch.load(
            f"{global_args['save_folder_name']}/{chk_files[-1]}", weights_only=False
        )
