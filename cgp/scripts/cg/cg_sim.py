...
def main(cfg):
    ...
    if len(chk_files) > 0:
        ...
        init_x = torch.randn(global_args["batch_size"], num_atoms, 3, requires_grad=True)
        init_v = None
        start_chk = 0
    else:
    ...
    integrator = OVRVO(u_model, masses, batch_size = global_args["batch_size"], **cfg["integrator_args"])
