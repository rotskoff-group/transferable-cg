...
import torch
import mdtraj as md
from cgp.cg import (get_bond_distance_features, 
...
class CGSimModel(UModel):
    def __init__(self, nn_model_name, nn_model_args,
                 ...
                 bias_force = None, 
                 **kwargs):
        super().__init__(nn_model_name, nn_model_args, 
                         prior_model_name, prior_model_args,
                         **kwargs)
    ...
    def store_features(self, batch_size, pdb_file, nn_config, prior_config, edge_args):
        ...
        length_indices, length_features = get_bond_distance_features(cg_topology, batch_size=0, as_tensor=True, device=self.device)
        angle_indices, angle_features = get_bond_angle_features(cg_topology, batch_size=0, as_tensor=True, device=self.device)
        dihedral_indices, dihedral_features = get_dihedral_angle_features(cg_topology, batch_size=0, as_tensor=True, device=self.device)
        atom_features = get_atom_features(cg_topology, batch_size=0, as_tensor=True, device=self.device)
        all_nonbonded_indices, all_nonbonded_features = get_nonbonded_features(cg_topology, batch_size=0, as_tensor=True, device=self.device)
        
        if "Transformer" in nn_config["model"]:
            esm2_embedding = get_esm2_embeddings(cg_topology, batch_size=0, as_tensor=True, device=self.device)
        else:
            esm2_embedding = None
        ...
        (length_indices, length_features,
         angle_indices, angle_features,
         dihedral_indices, dihedral_features,
         atom_features, 
         all_nonbonded_indices, all_nonbonded_features,
         esm2_embedding,
         edges) = map(lambda x: repeat_tensor(x, batch_size), 
                      (length_indices, length_features,
                       angle_indices, angle_features,
                       dihedral_indices, dihedral_features,
                       atom_features, 
        ...
        self.features = collate_fn({"batch_size" : batch_size,
                                    ...
                                    "esm2_embeddings": esm2_embedding,
    ...
    def get_force(self, pos):
        ...
        if self.nn.return_forces:
            ...
            if self.bias_force is not None:
                ...
                bias_force = -torch.autograd.grad(bias_force, pos, create_graph=True, 
                                                  grad_outputs=torch.ones_like(bias_force))[0]
                force += bias_force
        else:
            if self.bias_force is not None: 
                u_x += self.bias_force(pos)
            force = - torch.autograd.grad(u_x, pos, create_graph=True,
