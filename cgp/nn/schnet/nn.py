...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, atom_sctr_indices):
        ...
        offset = torch.cat(
            [
                torch.tensor([0]).to(atom_sctr_indices.device),
                torch.cumsum(counts, 0)[:-1],
            ]
        )
        indices = torch.arange(total).to(
            atom_sctr_indices.device
        ) - offset.repeat_interleave(counts)
...
class ExpNormalBasis(nn.Module):
    def __init__(self, n_basis, cutoff):
        ...
        self.register_buffer(
            "centers", torch.linspace(torch.exp(-self.cutoff), 1.0, n_basis)
        )
    ...
    def forward(self, distances):
        ...
        expanded = torch.exp(
            -self.beta * (torch.exp(-self.alpha * distances) - self.centers) ** 2
        )
...
class SchNetInteraction(nn.Module):
    def __init__(
        self, n_atom_basis, n_radial_basis, n_filters, include_bond_embedding=False
    ):
        super().__init__()
    ...
    def embed_bond(self, nonbonded_edge_indices):
        edges = (
            torch.abs(nonbonded_edge_indices[:, 0] - nonbonded_edge_indices[:, 1]) - 1
        )
        edges[edges > 2] = 3
    ...
    def forward(self, x, f_ij, nonbonded_edge_indices):
        ...
        w = (
            y[nonbonded_edge_indices[:, 1], :] * w
        )  # combining atom embedding and bond information
        output.scatter_add_(
            0, nonbonded_edge_indices[:, 0].unsqueeze(-1).expand(-1, w.shape[1]), w
        )
...
class SchNet(nn.Module):
    def __init__(
        ...
        if include_positional_encoding:
            self.positional_encoding = PositionalEncoding(
                n_atom_basis, max_len=5000, dropout_p=0.1
            )
        else:
    ...
    def forward(
        ...
        pair_dist = torch.linalg.norm(
            atomic_positions[edge_indices[:, 0], :]
