...
def compute_divergence_vmap(
    ...
    try:
        ...
        mask = torch.eye(score.shape[0], device=score.device, dtype=score.dtype).unsqueeze(-1)
...
def hutchinson_trace(score, x, num_samples=10, vectorize=False):
    if vectorize:
        v = (torch.randint(0, 2, (num_samples, *x.shape), device=x.device).float() * 2 - 1)
...
def get_eval_fn(self):
    if self.nn.return_forces:
        def get_force(pos, features, batch_size):
            ...
            if "return_load_balance" in features["nn_features"].keys() and features["nn_features"]["return_load_balance"]:
        ...
        def get_force(pos, features, batch_size):
            ...
            if "return_load_balance" in features["nn_features"].keys() and features["nn_features"]["return_load_balance"]:
                ...
                force = -grad(energy, pos, create_graph=True, retain_graph=True,
    ...
    if self.hparams.loss_type == "force matching":
        if self.hparams.load_balance_lambda > 0:
            if self.hparams.l1_lambda > 0:
                def loss(batch, batch_idx, prefix):
                    ...
                    pred_forces, load_balance_loss = get_force(pos, features, batch_size)
                    ...
                    metrics = {f"{prefix}/MSEForce": mse_force_loss,
                ...
                def loss(batch, batch_idx, prefix):
                    ...
                    pred_forces, load_balance_loss = get_force(pos, features, batch_size)
                    mse_force_loss = ((pred_forces - target_forces) ** 2).sum(-1).mean()
                    total_loss = mse_force_loss + self.hparams.load_balance_lambda * load_balance_loss
                    metrics = {f"{prefix}/MSEForce": mse_force_loss,
                                f"{prefix}/LoadBalance": load_balance_loss,
                                f"{prefix}/TotalLoss": total_loss,
                                "hp_metric": total_loss}
        ...
        if self.hparams.l1_lambda > 0.0:
            def loss(batch, batch_idx, prefix):
                ...
                pos.requires_grad = True
                get_score = lambda x: (get_force(x, features, batch_size)) * beta
                score = get_score(pos)
                ...
                metrics = {f"{prefix}/ScoreLoss": score_loss,
                            f"{prefix}/RegL1": reg_l1_loss,
                            f"{prefix}/TotalLoss": total_loss,
                            "hp_metric": total_loss}
                self.log_dict(metrics, on_epoch=True, 
