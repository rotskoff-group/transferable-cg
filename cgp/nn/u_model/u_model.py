...
class UModel(L.LightningModule):
    def __init__(
        ...
        super().__init__()
        assert not (nn_model_name is None and prior_model_name is None)
        self.use_nn = nn_model_name is not None
        self.use_prior = prior_model_name is not None
        self.nn = CGModel(
            nn_model_name,
            nn_model_args,
            prior_model_name,
        ...
        if isinstance(layers_to_freeze, str):
    ...
    def test_step(self, batch, batch_idx):
        ...
        metrics = {"test/MSEForce": mse_force_loss}
        self.log_dict(
            metrics,
            on_epoch=True,
    ...
    def configure_optimizers(self):
        ...
        u_optimizer = getattr(torch.optim, self.hparams.optimizer)
        optimizer_args = []
        if self.use_nn:
        ...
        if self.use_prior:
            optimizer_args.append(
                {
                    "params": self.nn.prior.parameters(),
                    "lr": self.hparams.optimizer_args.prior_lr,
                }
            )
        u_optimizer = u_optimizer(optimizer_args)
        u_scheduler = getattr(torch.optim.lr_scheduler, self.hparams.lr_scheduler)
        u_scheduler = u_scheduler(u_optimizer, **self.hparams.lr_scheduler_args)
        to_return = {
            "optimizer": u_optimizer,
            "lr_scheduler": u_scheduler,
    ...
    def load_model_from_ckpt(self, filename):
        ...
        model_weights = {
            k[3:]: v for k, v in all_weights.items() if k.startswith("nn.")
        }
        self.nn.load(
            model_weights, self.hparams.nn_model_name, self.hparams.nn_model_args
        )
