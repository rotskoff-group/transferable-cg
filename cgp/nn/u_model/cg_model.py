...
class CGModel(nn.Module):
    def __init__(
        ...
        assert not (nn_model_name is None and prior_model_args is None)

        if nn_model_name is None:  # No neural network, only prior
        ...
        if prior_model_name is None:  # No prior model, only neural network
    ...
    def forward(self, x, nn_features, prior_features):
        ...
        """
        if (
            "return_load_balance" in nn_features.keys()
            and nn_features["return_load_balance"]
        ):
            u_nn, loss = self.nn(x, **nn_features)
            u_prior = self.prior(x, **prior_features)
            return u_nn + u_prior, loss
        return self.nn(x, **nn_features) + self.prior(
            x, **prior_features
        )  ##TODO: Fix with load balance loss
            ...
            if model_name == "MACE":
                ...
                if model_params["use_cueq"]:
                    model_weights = cgp.nn.mace.tools.convert_e3nn_cueq.convert_weights(
                        model_weights, target_dict, model_params
                    )
                else:
