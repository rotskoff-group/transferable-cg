...
def forcefield_collate_features(
    ...
    **kwargs,
    ...
    bond_distances_sctr_indices = torch.cat(
        [
            torch.zeros(len(bond_distance_features[i]), dtype=torch.int64) + i
            for i in range(batch_size)
        ]
    )

    bond_distance_indices = [
        bond_distance_indices[i] + num_atoms_cum[i] for i in range(batch_size)
    ]
    ...
    angle_sctr_indices = torch.cat(
        [
            torch.zeros(len(angle_features[i]), dtype=torch.int64) + i
            for i in range(batch_size)
        ]
    )
    angle_indices = [angle_indices[i] + num_atoms_cum[i] for i in range(batch_size)]
    angle_indices = torch.cat(angle_indices, dim=0).type(torch.int64)
    angle_features = torch.cat(angle_features, dim=0)

    dihedral_sctr_indices = torch.cat(
        [
            torch.zeros(len(dihedral_features[i]), dtype=torch.int64) + i
            for i in range(batch_size)
        ]
    )
    dihedral_indices = [
        dihedral_indices[i] + num_atoms_cum[i] for i in range(batch_size)
    ]
    dihedral_indices = torch.cat(dihedral_indices, dim=0).type(torch.int64)
    dihedral_features = torch.cat(dihedral_features, dim=0)

    nonbonded_edge_sctr_indices = torch.cat(
        [
            torch.zeros(len(nonbonded_edge_features[i]), dtype=torch.int64) + i
            for i in range(batch_size)
        ]
    )

    nonbonded_edge_indices = [
        nonbonded_edge_indices[i] + num_atoms_cum[i] for i in range(batch_size)
    ]
    ...
    return {
        ...
        "bond_distances_sctr_indices": bond_distances_sctr_indices.to(
            bond_distance_indices.device
        ),
        ...
        "nonbonded_edge_sctr_indices": nonbonded_edge_sctr_indices.to(
            nonbonded_edge_indices.device
        ),
