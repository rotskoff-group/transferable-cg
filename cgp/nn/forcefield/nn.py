...
class ForceField(nn.Module):
    def __init__(
        ...
        if init_bonded_terms:
            ...
            init_bonded_terms = np.load(init_bonded_terms, allow_pickle=True).item()
            self.bonds_mean = nn.Parameter(
                torch.tensor(init_bonded_terms["bonds_mean"]).float()
            )
            self.bonds_k = nn.Parameter(
                torch.tensor(init_bonded_terms["bonds_k"]).float()
            )
        else:
        ...
        if init_angle_terms:
            ...
            init_angle_terms = np.load(init_angle_terms, allow_pickle=True).item()
            self.angles_mean = nn.Parameter(
                torch.tensor(init_angle_terms["angles_mean"]).float()
            )
            self.angles_k = nn.Parameter(
                torch.tensor(init_angle_terms["angles_k"]).float()
            )
        else:
        ...
        if init_dihedral_terms:
            ...
            init_dihedral_terms = np.load(init_dihedral_terms, allow_pickle=True).item()
            self.dihedral_a1 = nn.Parameter(
                torch.tensor(init_dihedral_terms["dihedral_a1"]).float()
            )
            self.dihedral_a2 = nn.Parameter(
                torch.tensor(init_dihedral_terms["dihedral_a2"]).float()
            )
        else:
            num_dihedral_terms = len(all_dihedral_angle_types)
            self.dihedral_a1 = nn.Parameter(
                torch.randn(num_dihedral_terms, num_dihedral_basis) / 1000
            )
            self.dihedral_a2 = nn.Parameter(
                torch.randn(num_dihedral_terms, num_dihedral_basis) / 1000
            )
        ...
        if init_nonbonded_terms:
            ...
            init_nonbonded_terms = np.load(
                init_nonbonded_terms, allow_pickle=True
            ).item()
            self.lj_r = nn.Parameter(torch.tensor(init_nonbonded_terms["lj_r"]).float())
            self.lj_epsilon = nn.Parameter(
                torch.tensor(init_nonbonded_terms["lj_epsilon"]).float()
            )
            self.charge = nn.Parameter(
                torch.tensor(init_nonbonded_terms["charge"]).float()
            )
            self.lj_r_1_4 = nn.Parameter(
                torch.tensor(init_nonbonded_terms["lj_r_1_4"]).float()
            )
            self.lj_epsilon_1_4 = nn.Parameter(
                torch.tensor(init_nonbonded_terms["lj_epsilon_1_4"]).float()
            )
            self.charge_1_4 = nn.Parameter(
                torch.tensor(init_nonbonded_terms["charge_1_4"]).float()
            )
        else:
            ...
            self.lj_epsilon_1_4 = nn.Parameter(
                torch.abs(torch.rand(num_nonbonded_terms))
            )
            self.charge_1_4 = nn.Parameter(torch.rand(num_nonbonded_terms) / 1000)

        if init_calvados_vdw_terms:
            init_calvados_vdw_terms = (
                f"{folder_path}/init_terms/init_calvados_vdw_terms.npy"
            )
            init_calvados_vdw_terms = np.load(
                init_calvados_vdw_terms, allow_pickle=True
            ).item()
            print("Initializing calvados vdw terms from file", flush=True)
            self.calvados_lambdas = nn.Parameter(
                torch.tensor(init_calvados_vdw_terms["calvados_lambdas"]).float()
            )
            self.calvados_sigmas = nn.Parameter(
                torch.tensor(init_calvados_vdw_terms["calvados_sigmas"]).float()
            )
            self.calvados_epsilon = nn.Parameter(
                torch.tensor(init_calvados_vdw_terms["calvados_epsilon"]).float()
            )
        else:
            num_ca = 21
    ...
    def forward(
        ...
        bond_distances_sctr_indices,
        angle_indices,
        angle_features,
        angle_sctr_indices,
        dihedral_indices,
        dihedral_features,
        dihedral_sctr_indices,
        ...
        if self.has_angles:
            ...
            angle_energy = (
                0.5 * k_per_bond_angle * ((angles - mean_per_bond_angle) ** 2)
            )
        ...
        if self.has_dihedrals:
            ...
            dihedrals_repeat = dihedrals.repeat_interleave(
                self.num_dihedral_basis, dim=1
            )
            dihedral_period = self.dihedral_period.repeat(dihedral_indices.shape[0], 1)
            dihedral_a1 = self.dihedral_a1[dihedral_features]
            dihedral_a2 = self.dihedral_a2[dihedral_features]
            dihedral_energy = torch.sum(
                dihedral_a1 * (torch.cos(dihedral_period * dihedrals_repeat))
                + dihedral_a2 * (torch.sin(dihedral_period * dihedrals_repeat)),
                dim=-1,
            )
        ...
        if self.has_vdw or self.has_coulomb or self.has_wca:
            if update_nonbonded_edges:
                ...
                pairwise_distance = torch.linalg.norm(
                    x[nonbonded_edge_indices[:, 0], :]
                    - x[nonbonded_edge_indices[:, 1], :],
                    dim=-1,
                )
            mask_1_4 = (
                nonbonded_edge_indices[:, 1] - nonbonded_edge_indices[:, 0]
            ) == 4
            ...
            if self.has_coulomb:
                charge = self.charge[atom_feature_1] * self.charge[atom_feature_2]
                charge_1_4 = (
                    self.charge_1_4[atom_feature_1] * self.charge_1_4[atom_feature_2]
                )
                ...
                energy.scatter_add_(0, nonbonded_edge_sctr_indices, coulomb_energy)

            if self.has_vdw or self.has_wca:
                sigma = (
                    torch.abs(self.lj_r[atom_feature_1])
                    + torch.abs(self.lj_r[atom_feature_2])
                ) / 2
                sigma_1_4 = (
                    torch.abs(self.lj_r_1_4[atom_feature_1])
                    + torch.abs(self.lj_r_1_4[atom_feature_2])
                ) / 2
                sigma[mask_1_4] = sigma_1_4[mask_1_4]

                epsilon = torch.abs(
                    self.lj_epsilon[atom_feature_1] * self.lj_epsilon[atom_feature_2]
                )
                epsilon_1_4 = torch.abs(
                    self.lj_epsilon_1_4[atom_feature_1]
                    * self.lj_epsilon_1_4[atom_feature_2]
                )
                epsilon[mask_1_4] = epsilon_1_4[mask_1_4]

                vdw_energy = epsilon * (
                    (sigma / pairwise_distance) ** 12
                ...
                if self.has_wca:  # TODO: replaced torch.where with sigmoid for gradient, not quite wca
                    ...
                    # nonbonded_edge_sctr_indices = nonbonded_edge_sctr_indices[mask]
                    energy.scatter_add_(
                        0, nonbonded_edge_sctr_indices[mask], vdw_energy
                    )
                else:
        ...
        if self.has_calvados_vdw:
            ...
            mask_ca = (((atom_feature_1 - 1) % 3 == 0) & (atom_feature_1 <= 61)) & (
                ((atom_feature_2 - 1) % 3 == 0) & (atom_feature_2 <= 61)
            )
            ...
            # calvados vdw
            sigma = (
                self.calvados_sigmas[atom_feature_1]
                + self.calvados_sigmas[atom_feature_2]
            ) / 2
            lambdas = (
                self.calvados_lambdas[atom_feature_1]
                + self.calvados_lambdas[atom_feature_2]
            ) / 2
            lj_energy = (
                4
                * self.calvados_epsilon
                * ((sigma / pairwise_distance) ** 12 - (sigma / pairwise_distance) ** 6)
            )
            lj_cutoff = (
                4
                * self.calvados_epsilon
                * (
                    (sigma / self.cutoff_distance) ** 12
                    - (sigma / self.cutoff_distance) ** 6
                )
            )
            ...
            vdw_energy[mask] = (
                lj_energy[mask]
                - (lambdas[mask] * lj_cutoff[mask])
                + (self.calvados_epsilon * (1 - lambdas[mask]))
            )
        ...
        if self.return_forces:
            forces = -torch.autograd.grad(
                energy, x, create_graph=True, grad_outputs=torch.ones_like(energy)
            )[0]
            return forces
        return energy
    ...
    def l1_loss(self):
        l1_loss = (
            (torch.abs(self.dihedral_a1) + torch.abs(self.dihedral_a2)).mean(0).sum()
        )
