...
def get_collate_fn(
    nn_config, prior_config, requires_forces=True, requires_energies=False
):
    assert not (nn_config is None and prior_config is None)
    ...
    def collate_features_fn(data):
        return {
            "nn_features": nn_collate_fn(**data),
            "prior_features": prior_collate_fn(**data),
        }
    ...
    def collate_fn(data):
        ...
        return ([x_positions, features], y_f, y_e, batch_size)
...
def get_hdf5_fn(cg_dataset_filename, individual_protein_dataset):
    ...
    """
    if individual_protein_dataset is not None:

        def get_hdf5_data():
            protein_names = np.load(individual_protein_dataset)
            return [
                h5py.File(f"{cg_dataset_filename}/{p}/dataset.hdf5", "r")
...
def create_dataset_from_path(
    ...
    data_in_memory,
    edge_args,
    individual_protein_dataset=None,
    ...
    if individual_protein_dataset is None:
        dataset_path = f"{dataset_folder_name}/dataset.hdf5"
    else:
        dataset_path = f"{dataset_folder_name}"

    get_hdf5_data = get_hdf5_fn(dataset_path, individual_protein_dataset)
    dataset = dataset(
        get_hdf5_data=get_hdf5_data,
...
def create_dataloaders(
    ...
    train, val, test = torch.utils.data.random_split(
        dataset, list(dataset_config["dataset_split_args"].values())
    )
    collate_fn, _ = get_collate_fn(
        nn_config,
        prior_config,
        requires_forces=requires_forces,
        requires_energies=requires_energies,
    ...
    if train_dataset_fraction != 1.0:
        subsample_indices_train = np.random.choice(
            np.arange(len(train)),
            int(len(train) * train_dataset_fraction),
            replace=False,
        )
        subsample_indices_val = np.random.choice(
            np.arange(len(val)), int(len(val) * train_dataset_fraction), replace=False
        )
    ...
    if adaption_dataset is not None:
        adaption_train, adaption_val, adaption_test = torch.utils.data.random_split(
            adaption_dataset, list(dataset_config["dataset_split_args"].values())
        )
