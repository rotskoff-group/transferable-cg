...
def transfer_symmetric_contractions(
    ...
    for i, kmax in kmax_pairs:
        ...
        wm = source_dict[f"nn.products.{i}.symmetric_contractions.weight"]
        ...
        for k in range(kmax+1):
            ...
            target_dict[
                f"nn.products.{i}.symmetric_contractions.contractions.{k}.weights.0"
            ] = weights_split[idx + 1]
            target_dict[
                f"nn.products.{i}.symmetric_contractions.contractions.{k}.weights.1"
            ] = weights_split[idx + 2]
            idx += 3
    return target_dict
