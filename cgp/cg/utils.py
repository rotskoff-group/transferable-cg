...
def create_cg_model(save_folder_name, root_data_folder_name, cg_config):
    ...
    cg_model = cg_model(
        save_folder_name, root_data_folder_name=root_data_folder_name, **cg_model_config
    )
    return cg_model


def concatenate_cg_datasets(
    all_sp_filenames, root_save_folder_name, store_in_dataset_args
):
    ...
    assert store_in_dataset_args["store_positions"]

    num_data_points_per_dataset = [
        h5_file["positions"].shape[0] for h5_file in all_h5py_files
    ]
    num_data_points = sum(num_data_points_per_dataset)
    num_data_points_per_dataset_cum = np.append(
        0, np.cumsum(num_data_points_per_dataset)
    )

    num_cg_beads_per_dataset = np.array(
        [h5_file["positions"].shape[1] for h5_file in all_h5py_files]
    )
    ...
    if store_in_dataset_args["store_positions"]:
        file.create_dataset("positions", (num_data_points, max_cg_beads, 3), dtype="f4")
    if store_in_dataset_args["store_forces"]:
        file.create_dataset("forces", (num_data_points, max_cg_beads, 3), dtype="f4")
    if store_in_dataset_args["store_energies"]:
        file.create_dataset("energies", (num_data_points, 1), dtype="f4")

    if store_in_dataset_args["store_features"]:
        num_features = [h5_file["features"].shape[2] for h5_file in all_h5py_files]
        assert np.unique(num_features).shape[0] == 1
        num_features = num_features[0]
        file.create_dataset(
            "features",
            (num_data_points, max_cg_beads, num_features),
            dtype="i4",
            fillvalue=-1,
        )
    ...
    if store_in_dataset_args["store_atom_features"]:
        ...
        file.create_dataset(
            "atom_features", (num_data_points, max_cg_beads), dtype="i4", fillvalue=-1
        )
    ...
    if store_in_dataset_args["store_bond_distance_features"]:
        ...
        file.create_dataset(
            ...
            (num_data_points, max_num_bonds),
            dtype="i4",
            fillvalue=-1,
        )
        file.create_dataset(
            "bond_distance_indices",
            (num_data_points, max_num_bonds, 2),
            dtype="i4",
            fillvalue=-1,
        )
    ...
    if store_in_dataset_args["store_bond_angle_features"]:
        ...
        file.create_dataset(
            "angle_features",
            (num_data_points, max_num_angles),
            dtype="i4",
            fillvalue=-1,
        )
        file.create_dataset(
            "angle_indices",
            (num_data_points, max_num_angles, 3),
            dtype="i4",
            fillvalue=-1,
        )
    ...
    if store_in_dataset_args["store_dihedral_angle_features"]:
        ...
        file.create_dataset(
            "dihedral_features",
            (num_data_points, max_num_dihedral_angles),
            dtype="i4",
            fillvalue=-1,
        )
        file.create_dataset(
            "dihedral_indices",
            (num_data_points, max_num_dihedral_angles, 4),
            dtype="i4",
            fillvalue=-1,
        )

    if store_in_dataset_args["store_nonbonded_features"]:
        num_pairwise_per_dataset = [
            h5_file["nonbonded_edge_features"].shape[1] for h5_file in all_h5py_files
        ]
        max_num_pairwise = max(num_pairwise_per_dataset)
        file.create_dataset(
            "nonbonded_edge_features",
            (num_data_points, max_num_pairwise, 2),
            dtype="i4",
            fillvalue=-1,
        )
        file.create_dataset(
    ...
    for i in range(len(all_h5py_files)):
        ...
        if store_in_dataset_args["store_positions"]:
            ...
            da.store(
                cg_positions,
                file["positions"],
                regions=(slice(start, end), slice(0, int(num_cg_beads_per_dataset[i]))),
            )
        if store_in_dataset_args["store_forces"]:
        ...
        if store_in_dataset_args["store_energies"]:
            ...
            da.store(
                cg_energies, file["energies"], regions=(slice(start, end), slice(None))
            )
        if store_in_dataset_args["store_features"]:
        ...
        if store_in_dataset_args["store_bond_distance_features"]:
            cg_bond_distance_features = da.from_array(
                h5py_file["bond_distance_features"],
                chunks=(10, num_bonds_per_dataset[i]),
            )
            cg_bond_distance_indices = da.from_array(
                h5py_file["bond_distance_indices"],
                chunks=(10, num_bonds_per_dataset[i], 2),
            )
            da.store(
                cg_bond_distance_features,
                file["bond_distance_features"],
                regions=(slice(start, end), slice(0, int(num_bonds_per_dataset[i]))),
            )
            da.store(
                cg_bond_distance_indices,
                file["bond_distance_indices"],
                regions=(
                    slice(start, end),
        ...
        if store_in_dataset_args["store_bond_angle_features"]:
            cg_angle_features = da.from_array(
                h5py_file["angle_features"], chunks=(10, num_angles_per_dataset[i])
            )
            cg_angle_indices = da.from_array(
                h5py_file["angle_indices"], chunks=(10, num_angles_per_dataset[i], 3)
            )
            da.store(
                cg_angle_features,
                file["angle_features"],
                regions=(slice(start, end), slice(0, int(num_angles_per_dataset[i]))),
            )
            da.store(
                cg_angle_indices,
                file["angle_indices"],
        ...
        if store_in_dataset_args["store_dihedral_angle_features"]:
            cg_dihedral_features = da.from_array(
                h5py_file["dihedral_features"],
                chunks=(10, num_dihedral_angles_per_dataset[i]),
            )
            cg_dihedral_indices = da.from_array(
                h5py_file["dihedral_indices"],
                chunks=(10, num_dihedral_angles_per_dataset[i], 4),
            )
            da.store(
                cg_dihedral_features,
                file["dihedral_features"],
                regions=(
                    slice(start, end),
        ...
        if store_in_dataset_args["store_nonbonded_features"]:
            cg_pairwise_features = da.from_array(
                h5py_file["nonbonded_edge_features"],
                chunks=(10, num_pairwise_per_dataset[i], 2),
            )
            cg_pairwise_indices = da.from_array(
                h5py_file["nonbonded_edge_indices"],
                chunks=(10, num_pairwise_per_dataset[i], 2),
            )
            da.store(
                cg_pairwise_features,
...
def get_bond_distance_features(cg_topology, batch_size, as_tensor=False, device=None):
    all_bond_indices = [[i, i + 1] for i in range(cg_topology.n_atoms - 1)]
    all_bond_indices = np.array(all_bond_indices)
    all_bond_features = [
        all_bond_types[
            cg_topology.atom(atom_index_1).name, cg_topology.atom(atom_index_2).name
        ]
    ...
    if batch_size > 0:
        ...
        all_bond_features = np.repeat(all_bond_features, batch_size, axis=0)

        all_bond_indices = np.expand_dims(all_bond_indices, axis=0)
        all_bond_indices = np.repeat(all_bond_indices, batch_size, axis=0)
    if as_tensor:
...
def get_bond_angle_features(cg_topology, batch_size, as_tensor=False, device=None):
    all_angle_indices = [[i, i + 1, i + 2] for i in range(cg_topology.n_atoms - 2)]
    all_angle_indices = np.array(all_angle_indices)

    all_angle_features = [
        all_angle_types[
            amber_res_code_to_standard[cg_topology.atom(atom_index_1).residue.name],
    ...
    if batch_size > 0:
        ...
        all_angle_features = np.repeat(all_angle_features, batch_size, axis=0)

        all_angle_indices = np.expand_dims(all_angle_indices, axis=0)
        all_angle_indices = np.repeat(all_angle_indices, batch_size, axis=0)
    if as_tensor:
...
def get_dihedral_angle_features(cg_topology, batch_size, as_tensor=False, device=None):
    all_dihdral_indices = [
        [i, i + 1, i + 2, i + 3] for i in range(cg_topology.n_atoms - 3)
    ]
    all_dihdral_indices = np.array(all_dihdral_indices)

    all_dihedral_features = [
        all_dihedral_angle_types[
            amber_res_code_to_standard[cg_topology.atom(atom_index_1).residue.name],
            amber_res_code_to_standard[cg_topology.atom(atom_index_2).residue.name],
            amber_res_code_to_standard[cg_topology.atom(atom_index_3).residue.name],
...
def get_atom_features(cg_topology, batch_size, as_tensor=False, device=None):
    ...
    all_atom_features = [
        all_pairwise_types[
            amber_res_code_to_standard[cg_topology.atom(i).residue.name],
            cg_topology.atom(i).name,
        ]
    ...
    if batch_size > 0:
        ...
        all_atom_features = np.repeat(all_atom_features, batch_size, axis=0)
...
def get_nonbonded_features(cg_topology, batch_size, as_tensor=False, device=None):
    ...
    if batch_size > 0:
        ...
        all_nonbonded_indices = np.repeat(all_nonbonded_indices, batch_size, axis=0)
        ...
        all_nonbonded_features = np.repeat(all_nonbonded_features, batch_size, axis=0)
...
def get_cg_bead_masses(cg_topology):
    ...
    for i in range(num_atoms):
        ...
        if atom.residue.index == 0 and res_name != "ACE" and atom_name == "N":
            ...
            atom.residue.index == num_residues - 1
            and res_name != "NME"
            and atom_name == "C"
        ):
            mass += o_mass
